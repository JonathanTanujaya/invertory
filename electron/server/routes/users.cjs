...
function registerUserRoutes(fastify, { db }) {
  ...
  fastify.post("/api/users", async (request, reply) => {
    ...
    if (role !== "admin")
      return reply.code(400).send({ error: "role must be admin" });
  ...
  fastify.put("/api/users/:id", async (request, reply) => {
    ...
    const isActive =
      body.isActive != null ? (body.isActive ? 1 : 0) : undefined;

    const existing = db.get("SELECT id, role FROM m_user WHERE id = ?", [id]);
    if (!existing) return reply.code(404).send({ error: "not found" });
    if (existing.role === "owner")
      return reply.code(400).send({ error: "cannot modify owner user" });
  ...
  fastify.delete("/api/users/:id", async (request, reply) => {
    ...
    if (!existing) return reply.code(404).send({ error: "not found" });
    if (existing.role === "owner")
      return reply.code(400).send({ error: "cannot delete owner user" });
  ...
  fastify.post("/api/users/:id/reset-password", async (request, reply) => {
    ...
    if (existing.role === "owner")
      return reply
        .code(400)
        .send({ error: "cannot reset owner password here" });
