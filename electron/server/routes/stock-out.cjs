...
function registerStockOutRoutes(fastify, { db }) {
  ...
  fastify.get('/api/stock-out/:id', async (request, reply) => {
  ...
  fastify.post('/api/stock-out', async (request, reply) => {
    ...
    try {
      const result = db.transaction((tx) => {
        ...
        for (const it of items) {
          ...
          const harga_jual = it.harga_jual ?? null;
...
module.exports = { registerStockOutRoutes };
